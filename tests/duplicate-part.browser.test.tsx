...
describe('Duplicate a Part (Cmd+D)', () => {
  ...
  it('Cmd+D new part has same X/Z and Y offset by 1 from the original', async () => {
    useProjectStore.getState().addPart({
      length: 6, width: 4,
    ...
    await renderInCanvas(<Scene selectedIds={[original.id]} onSelectIds={() => {}} />)
    await new Promise((resolve) => setTimeout(resolve, 50))
    ...
    expect(newPart.position.y).toBe(original.position.y + 1)
  })
